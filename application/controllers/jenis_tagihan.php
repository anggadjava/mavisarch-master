	...
	*/
	public function index()
	{
		if($this->session->userdata('logged_in')!="")
		{
			...
			$d['tot'] = $offset;
			$tot_hal = $this->app_model->getAllData("jenis_tagihan");
			...
			if (empty($_GET)) {
				$d['data'] = $this->app_model->getAllDataLimited("jenis_tagihan",$limit,$offset);
			}
			else{
				$d['data'] = $this->app_model->searchGetAllDataLimited("jenis_tagihan",$limit,$offset,array('NIK','nama'),$_GET['cari']);
			}
			
			$d['content']= $this->load->view('jenis_tagihan/daftar',$d,true);
	...
	public function tambah()
	{
		if($this->session->userdata('logged_in')!="")
		{
			$d['id'] = '';
			$d['nama_tagihan'] = '';
			$d['content']= $this->load->view('jenis_tagihan/form',$d,true);
			$this->load->view('home',$d);
		}
		else
		{
